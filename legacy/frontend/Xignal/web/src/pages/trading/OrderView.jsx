...
const OrderView = ({ id, getListData, presetData }) => {
	...
	const handleSave = useCallback(() => {
		...
		trading.liveDetailUpload(toPayload(), null, (res) => {
						...
						<div>
							...
							<div className="space-y-2">
								<div className="flex items-center gap-2">
									<CheckboxBig checked={formData.splitTakeProfitEnabled} id="splitTakeProfitEnabled-live" onChange={(value) => setFormData({
										...formData,
									...
									<div className="space-y-3 rounded-md border border-[#2D2D2D] bg-[#101010] p-3">
										...
										<div className="space-y-2">
											{createSplitStageList(Number(formData.splitTakeProfitCount || 1), formData.splitTakeProfitStages).map((stage, index) => (
												<div key={`live-split-stage-${index}`} className="grid grid-cols-[80px_1fr_1fr] items-center gap-2">
				...
				<div className={cardClass}>
					...
					<div className="grid grid-cols-1 gap-4 md:grid-cols-3">
						<div>
							...
							<div className="flex items-center gap-2">
								<CheckboxBig checked={formData.stopLoss !== ''} id="stopLossEnabled-live" onChange={(value) => setFormData({ ...formData, stopLoss: value ? formData.stopLoss || '3' : '' })} />
						...
						<div>
							...
							<div className="flex min-h-[44px] items-center">
								<CheckboxBig checked={formData.stopLossReverseEnabled} id="stopLossReverseEnabled-live" onChange={(value) => setFormData({ ...formData, stopLossReverseEnabled: value })} />
							</div>
						</div>
						<div>
							<div className={rowLabelClass}><p className="h-1 w-1 rounded-full bg-[#999]" />시간 경과</div>
							<div className="flex items-center gap-2">
								<CheckboxBig checked={formData.stopLossTimeEnabled} id="stopLossTimeEnabled-live" onChange={(value) => setFormData({ ...formData, stopLossTimeEnabled: value })} />
								<div className="relative flex-1">
