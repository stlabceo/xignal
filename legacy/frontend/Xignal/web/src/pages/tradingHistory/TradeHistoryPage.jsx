...
const TradeHistoryPage = () => {
	...
	useEffect(() => {
		if (tap === 'done') {
			...
			trading.getExactTrackRecord(params, (res) => {
				setProgressData(res);
			});
		}
	}, [tap, page]);
		...
		<div className="inner-container">
			<div className="mb-5 flex flex-col gap-3 md:mb-5.5 md:flex-row md:items-center md:gap-4">
				...
				<div className="flex items-center gap-2">
					<div className="rounded-sm bg-[#fff] px-2 text-[14px] font-bold md:text-[16px]">TRADING</div>
					<div className="rounded-sm border border-[#494949] px-2 text-[14px] font-bold text-[#fff] md:text-[16px]">DEMO</div>
				</div>
			</div>
					...
					<div className="flex flex-col gap-4">
						...
						<div className="md:hidden space-y-3">
							{progressData.map((item, idx) => (
								<div
									key={`progress_trade_mobile_${idx}`}
									className="rounded-lg border border-[#2d3340] bg-[#1B1B1B] p-4 text-white cursor-pointer"
									onClick={() => navigate(`/trade-history/${item.pid}`)}
								>
									<div className="mb-3 break-all text-[16px] font-semibold">{item.a_name}</div>
