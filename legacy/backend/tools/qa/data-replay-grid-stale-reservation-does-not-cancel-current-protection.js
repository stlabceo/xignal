...
const { runGridStaleReservationDoesNotCancelCurrentProtection } = require("./qa-scenarios");

const run = async () => {
  const config = loadQaConfig();
  const scenario = await runGridStaleReservationDoesNotCancelCurrentProtection({ uid: config.uid });
