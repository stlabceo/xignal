...
const formatRow = (scenario) => ({
  ...
  audit: (scenario.msgList || []).map((item) => `${item.code}:${item.fun}`).join(" || "),
...
const run = async () => {
  ...
  const scenarios = [];
  scenarios.push(await runSignalRecoveredCloseViaTruthSync({ uid: config.uid }));
  scenarios.push(await runGridRecoveredCloseViaReconcile({ uid: config.uid }));
