...
const run = async () => {
  ...
  const scenario = await runSignalSplitTpMultiFillExitAccounting({ uid: config.uid });
    ...
    [
      {
        ...
        row: JSON.stringify({
          ...
          r_exactTime: scenario.row?.r_exactTime || null,
        }),
  ...
  printQaSummary({
    ...
    rowSummary: [{
      scenario: scenario.scenario,
      row: `${scenario.row?.status || "-"} / ${Number(scenario.row?.r_qty || 0)} / ${Number(scenario.row?.r_exactPrice || 0)} / ${scenario.row?.r_exactTime || "-"}`,
    }],
    reservationSummary: [{
      scenario: scenario.scenario,
